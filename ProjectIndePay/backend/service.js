...
exports.getLastTransactions = function(data, callback) {
  ...
  var selectQuery =
    "SELECT T.SYS_CREATE_DATE 'DATE', T.AMOUNT 'AMOUNT', T.COMMENT 'COMMENT',T.FEE 'FEE', T.RECEIVER 'RECIVERID',CU.NAME 'CURRENCY',CA.NAME 'CATEGORY',RS.LOGINNAME 'SENDER',RR.LOGINNAME 'RECEIVER' FROM TRANSACTION T JOIN REGUSER RS ON T.SENDER = RS.USERID JOIN REGUSER RR ON T.RECEIVER = RR.USERID JOIN CURRENCY CU ON T.CURRENCY = CU.CURRENCYID JOIN CATEGORY CA ON T.CATEGORY = CA.CATEGORYID WHERE (SENDER = ? OR RECEIVER = ?) ORDER BY T.SYS_CREATE_DATE DESC LIMIT ?,?;";
  var query = mysql.format(selectQuery, data);
  var connection = createNewConnection(false);
  connection.connect();
  connection.query(query, function(err, rows, fields) {
